...
from ..mixin import ParameterizedRouteMixin, ListTestCaseMixin, CreateTestCaseMixin, ExporterTestCaseMixin, \
    ...
    PresentableTestCaseMixin, ExporterCustomFieldsMixin, CustomFieldsMixin, ListCreateChildTestCaseMixin
...
class TestNetworksByOrganizationView(
    ...
    def __send_list_request(self, user="user_1", input_uuid="POPULATE", query_string=None, login=True):
        """
        ...
        :param input_uuid: The UUID of the organization to request.
    ...
    def __send_create_request(
        ...
        :param input_uuid: The UUID of the organization to include in the request.
        ...
        if input_uuid == "POPULATE":
            organization = self.get_organization_for_user(user=user)
            input_uuid = str(organization.uuid)
        if include_name:
            to_send["name"] = name
    ...
    def test_create_unknown_org_uuid_fails(self):
        ...
        """
        org_uuid = str(uuid4())
        self.assert_request_not_found(self.send_create_request(input_uuid=org_uuid))
    ...
    def test_create_not_owned_org_uuid_fails(self):
        ...
        self.assert_request_fails(self.send_create_request(user="user_1", input_uuid=other_org.uuid))
    ...
    def test_create_not_owned_org_uuid_admin_success(self):
        ...
        response = self.send_create_request(user="admin_1", input_uuid=other_org.uuid)
...
class TestOrdersByOrganizationView(
    ...
    def __send_list_request(self, user="user_1", input_uuid="POPULATE", query_string=None, login=True):
        ...
        Send an HTTP request to the configured API endpoint and return the response.
        :param user: A string depicting the user to send the request as.
        :param input_uuid: The UUID of the organization to query orders for.
        :param query_string: The query string to include in the URL.
        :param login: Whether or not to log the requesting user in.
        :return: The HTTP response.
        """
