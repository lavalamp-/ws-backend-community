...
class TestOrderScanConfigDetailView(
    UpdateTestCaseMixin,
    RetrieveTestCaseMixin,
    PresentableTestCaseMixin,
    CustomFieldsMixin,
    ...
    """
    This is a test case for testing the OrderScanConfigDetailView view.
    """

    _api_route = "/orders/%s/scan-config/"
    _url_parameters = None
    ...
    def __send_retrieve_request(
        ...
        """
        Submit a request to the remote endpoint to retrieve the given scan config.
        ...
        :param input_uuid: The UUID of the parent order to retrieve the scan config from.
        :return: The HTTP response.
        """
        if login:
            self.login(user=user)
